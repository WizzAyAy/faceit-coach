...
import { analyzeLobby, analyzeTeam } from '../analyzer.js'
import { cache } from '../cache.js'

import { faceitApi } from '../faceit-api.js'

vi.mock('../../config.js', () => ({
  config: { faceitApiKey: 'test' },
}))

// Mock faceitApi
vi.mock('../faceit-api.js', () => ({
  faceitApi: {
    getPlayer: vi.fn(),
...
describe('analyzer integration', () => {
  ...
  describe('analyzeTeam', () => {
    it('should analyze a team of players', async () => {
      ...
      const result = await analyzeTeam(['p1', 'p2'], 20)
    ...
    it('should return empty array for empty playerIds', async () => {
      const result = await analyzeTeam([], 20)
  ...
  describe('analyzeLobby', () => {
    it('should analyze both teams and return pick/ban', async () => {
      ...
      const result = await analyzeLobby('m1', 1, 20)
