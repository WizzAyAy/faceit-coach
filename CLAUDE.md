...
- **Panneau draggable** : le header sert de poignee (pointer events, support souris + tactile). La position `{ top, left }` est persistee dans `browser.storage.local` sous la cle `panelPosition` (local et non sync car specifique a la resolution de l'ecran). Au chargement, si la position sauvegardee est hors viewport (resolution differente, ecran plus petit), elle est purgee et le panneau revient a l'ancrage par defaut. Pendant le drag, on clamp pour garder au moins `MIN_VISIBLE_PX = 80px` du panneau visible sur chaque axe. Un listener `resize` reset la position si elle devient invalide apres redimensionnement de la fenetre.
